...
static IO_t sdioPins[6];
#else
static IO_t sdioPins[3];
#endif
#if !defined(SD_SDIO_DMA_STREAM3) && !defined(SD_SDIO_DMA_STREAM6)
#define SD_SDIO_DMA_STREAM3
#endif

void SD_DiskInit(void) {
    ...
    sdioPins[0] = IOGetByTag(IO_TAG(PD2));
    sdioPins[1] = IOGetByTag(IO_TAG(PC8));
    sdioPins[2] = IOGetByTag(IO_TAG(PC12));
#if FATFS_SDIO_4BIT == 1
    sdioPins[3] = IOGetByTag(IO_TAG(PC9));
    sdioPins[4] = IOGetByTag(IO_TAG(PC10));
    sdioPins[5] = IOGetByTag(IO_TAG(PC11));
#endif
    for (uint8_t i = 0; i < sizeof(sdioPins) / sizeof(sdioPins[0]); i++)
        IOInit(sdioPins[i], OWNER_SYSTEM, RESOURCE_INPUT);
}
/*
...
void SD_LowLevel_DeInit(void) {
	...
	/* Disable the SDIO APB2 Clock */
        ...
        for (uint8_t i = 0; i < sizeof(sdioPins) / sizeof(sdioPins[0]); i++) {
            IOConfigGPIO(sdioPins[i], IOCFG_IPD);
        }
}
...
void SD_LowLevel_Init (void) {
        for (uint8_t i = 0; i < sizeof(sdioPins) / sizeof(sdioPins[0]); i++) {
            IOConfigGPIOAF(sdioPins[i], IO_CONFIG(GPIO_Mode_AF, GPIO_Fast_Speed, GPIO_OType_PP, GPIO_PuPd_UP), GPIO_AF_SDIO);
        }
